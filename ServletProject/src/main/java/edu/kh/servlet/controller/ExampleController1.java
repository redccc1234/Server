 ...
 * 
 * - http 프로토콜 서비스를 지원하는 추상 클래스-> 상속받아서 나만의 서블릿 클래스 구현
 * 
 * [servlet 등록 절차]
...
// 클라이언트로부터 "/example"이라는 요청을 받아서 처리한 후 
// 응답을 해줄 서블릿 클래스 ExampleController1 을 만든다
...
public class ExampleController1 extends HttpServlet {
	...
	// doGet() 메서드
	//-> get 방식 요청을 처리하는 메서드
	// HttpServlet 을 상속받으면 사용할 수 있는 메서드, 오버라이딩하여 사용
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// - 클라이언트 요청 시 생성되는 객체
		// - 요청 시 전달된 데이터, 요청한 클라이언트 정보, 유연한 요청 처리를 하기 위한 객체를 사용함.
		...
		// 2) 응답받은 내용(HTML)을 클라이언트의 브라우저가 해석하여 화면에 출력
		
		// HttpServletResponse 객체 이용
		
		// 응답처리 1 : 응답하는 문서의 형시과 문자 인코딩 지정
